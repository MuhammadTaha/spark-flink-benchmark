...
public class ArgumentValidator {
    ...
    public static boolean validArguments(String[] inputArgs) {
        ...
        if (inputArgs.length < 5 && inputArgs.length > 7 ) {
            System.out.println("Please run: java -jar benchmarkclient-1.0.jar <cluster_type> <metric_type> <s3_bucket_data_file> <s3_emr_log_dir> [data_size]");
            return false;
        }
        ...
        if (!Arrays.asList(METRIC_TYPES).contains(inputArgs[3])) {
            System.out.println("Please enter a valid metric type:\n<metric_types>: {\"groupby\", \"sorting\", \"aggregate\"}");
            return false;
        }
